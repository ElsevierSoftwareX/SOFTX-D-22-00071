% 
% MAIN for synthetic data creation V-V kernel
...
par.svd=0;%
delta=DELTA;
out_folder='./Synth_VV';mkdir(out_folder);
fprintf('------------------------------------------------------------------ \n')
fprintf(' Folder name %s \n',out_folder)
[Kc,Kr,s,gexact,par] =data2d_simul_2peaks_par(delta,1024, 128, 64,64);
...
grafico_2D(gexact,par.T1,par.T2,0, 1, 20, 'True')
...
[fmodel3,PK3]=modello_componente(sigma_T1_3,T1_exp,media_T1_3,sigma_T2_3,T2_exp,media_T2_3,pi/3);%Rotazione pi/6
...
Kernel_1 = inline('1-exp(- Tau * (1./ TimeConst))','Tau','TimeConst');
K1 = Kernel_1 (Tau1,T1);
Kernel_2 = inline('1-exp(- Tau * (1./ TimeConst))','Tau','TimeConst');
K2 = Kernel_2(Tau2,T2);
